...
'use strict';
//A simple GComp client implementation. Downloads jobs from the job server and executes them.
//This can be executed stand-alone as well and should be maintained as such to ensure
//continued multi-platform compatibility.
...
function Client() {
  ...
  this.jobclient = {
    ...
    //TODO: Not necessary if the job server certificate
    //is signed by a recognized signing authority.
    ca: this.client_consts.JOB_SERVER_CACERT
  };
...
Client.prototype.setConsts = function(newConsts) {
  ...
  this.jobclient.ca = this.client_consts.JOB_SERVER_CACERT;
};
...
Client.prototype.getJobDescription = function(reqOptions, callback) {
  if (!reqOptions) {
    reqOptions = {
      ...
      //Setup SSL specification
      agent: false,
      key: this.jobclient.key,
      cert: this.jobclient.cert,
      passphrase: this.jobclient.passphrase,
      ca: this.jobclient.ca,
...
Client.prototype.getJob = function(jobDesc, attributes, headers, callback) {
  ...
  var req = https.request({
    ...
    //Identify this client as one approved by the job server,
    //seeing as the client got the keys from the job server
    //over an encrypted connection.
    key: jobDesc.client_key,
    cert: jobDesc.client_cert,

    //Predistributed PM CA certificate to identify valid PM servers.
    ca: [fs.readFileSync(resPath + 'pmserver-cacert.pem').toString()],

    //Disallow servers that do not have a certificate signed by the job server.
    rejectUnauthorized: true
  }, function(res) {
    if (res.statusCode != self.client_consts.STATUS_CODE_OK) {
      consolelog('statusCode: ' + res.statusCode);
  ...
  zlib.gzip(data, function(err, compressed) {
    if (err) {
      ...
      var options = {
        ...
        //Setup SSL specification
        agent: false,
        key: self.jobclient.key,
        cert: self.jobclient.cert,
        passphrase: self.jobclient.passphrase,
        ca: self.jobclient.ca,
        rejectUnauthorized: true
      };
...
Client.prototype.actionKey = function(key_data) {
  if (key_data && key_data.key !== '[no-key]') {
    this.jobclient.passphrase = key_data.key;
  } else {
    this.jobclient.passphrase = null;
  }
  ...
  if (this.jobclient.passphrase) {
    consolelog('Decrypting keys...');
    //These certs are signed by the job server client CA.
    this.jobclient.key = fs.readFileSync(resPath + 'client-key.pem');
    this.jobclient.cert = fs.readFileSync(resPath + 'client-cert.pem');
  } else {
    consolelog('No key passhphrase, running keyless.');
    //Here, no passphrase could be supplied, so no client certificate is available.
    this.jobclient.key = null;
    this.jobclient.cert = null;
  }
  // Notify key accepted.
